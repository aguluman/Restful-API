...
//Instantitate the CLI module object
var cli = {};
...
cli.responders.help = function(){
    var commands = {
        ...
        'help' : 'Alias of the "Man (manual)" command',
        'stats' : 'Get the statistics on the underlying operating system and resource utilisation',
        'list users' : 'Show a list of all the registered and (undeleted) users in the system',
    ...
    //Show each command, followed bu=y its explanation, in white and yellow respectively
    for(var key in commands){
        if(commands.hasOwnProperty(key)){
            var value = commands[key];
            var line  = '\x1b[33m'+key+'\x1b[0m';
            var padding = 45 - line.length;
            for(i=0; i < padding; i++){
                line += ' ';
            }
            line += value;
...
cli.centered = function(str){
    ...
    //Put in left padded spaces before teh string itself
    var line = '';
        ...
        if(!err && checkIds && checkIds.length > 0){
            ...
            checkIds.forEach(function(checkId){
                _data.read('checks', checkId, function(err, checkData){
                    ...
                    //Get the state of the checks but default to down
                    ...
                    //If the user has specified the state, or has not specified any state, include the current check accordingly
...
cli.init = function(){
    ...
    //If the user stops the CLI, kill the asssociated process
